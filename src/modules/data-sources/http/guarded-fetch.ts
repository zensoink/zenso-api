...
async function assertUrlAllowed(url: URL, isEntry: boolean): Promise<void> {
...
export async function guardedFetch(inputUrl: string): Promise<string> {
  let currentUrl = new URL(inputUrl);

  for (let hop = 0; hop <= MAX_REDIRECTS; hop++) {
    await assertUrlAllowed(currentUrl, hop === 0);
